...
このアプリの本質は、AIが「どこを直すべきか」を判断し、既存LPのデザインを壊さずに自然なHTMLへ落とすことにある。差し込み型だけでは改善案の確認はできるが、実LPとしての完成度は上がりにくい。今後は、置換できる箇所は置換し、置換が危険な箇所だけ差し込みに戻す。
...
- 置換した場合は `type=replace_copy` または `type=replace_cta_copy` として残す
- `before` に変更前の文言を残す
- `after` に変更後の文言を残す
- `target_area` と `target_section` を残す
- 置換できず差し込んだ場合は `type=direct_block` として残す
...
- オファー・価格価値の置換ルール
- 口コミ・実績を事実確認付きで扱うルール
- 既存HTMLの構造別、画像中心LPとテキスト中心LPの編集方針
