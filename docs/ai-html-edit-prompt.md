...
## 自然なHTML/CSS書き換え品質ルール

AI編集は「AIが足した説明ブロック」に見えないことを重視する。既存LPの一部として読めるように、現在の見出し階層、文章量、CTAの言い回し、余白、色、レスポンシブの作法に合わせる。

### 置換の基本方針

- 既存の強い構造を活かす
  - 既存の `section`、`h1`、`h2`、CTAボタン、カード、FAQ項目を優先して使う。
  - 新しい大きなセクションを増やす前に、既存セクション内の見出し・リード・補足文を置換する。
- 文量を増やしすぎない
  - heroは短く、CTA周辺は1〜2文、FAQは1項目単位で足す。
  - スマホで見たときに1画面が文章だけで埋まる変更は避ける。
- 既存トーンに寄せる
  - 高級感、やわらかさ、専門性、親しみやすさなど、既存LPの口調を壊さない。
  - 急に煽りが強い言葉、広告臭が強い言葉、断定的な実績表現を入れない。
- CTAは行動の不安を減らす
  - 「予約する」だけでなく、相談で何が分かるか、初回で何を確認できるかを近くに添える。
  - ただしリンク先、電話番号、LINE URL、外部予約URLは変更しない。
- CSSは既存部品に寄せる
  - 既存の色、角丸、影、余白、フォントサイズを観察して、小さなhelper classだけ使う。
  - グローバルCSSや全体レイアウトには触らない。

### target_area別の自然な編集

| target_area | 優先編集 | 避ける編集 |
| --- | --- | --- |
| hero | 見出しの約束、対象者、得られる変化を短く補強 | 長い説明文、実績の断定、デザインを覆う大きなブロック |
| cta | ボタン近くに相談のハードルを下げる1文を追加 | リンク変更、過度な煽り、CTAが増えすぎる変更 |
| offer | 価格価値、初回の分かりやすさ、選ぶ理由を補足 | 値引きや保証など未確認の約束 |
| proof | 既存の実績・口コミ・before afterの見せ方を整理 | 架空の口コミ、No.1、医療的断定 |
| faq | 予約前不安、所要時間、痛み、持ち物などを1項目ずつ補強 | 長すぎるFAQ、LP外の運用ルールの断定 |
| measurement | HTML変更ではなく、イベント・リンク・GTM確認として記録 | 計測不備をコピー改善で解決した扱いにする |

### 自然さの自己レビュー

HTML編集案を出す前に、以下を必ず確認する。

- 既存HTMLのどの要素を置換するかが明確か
- 置換後の文量が既存要素の文量から大きく外れていないか
- スマホ表示で読める長さか
- 既存CTAの意味とリンク先を変えていないか
- 事実確認が必要な表現を新しく作っていないか
- GA4の課題に対して、変更箇所が遠すぎないか
- 変更前後の `before` / `after` がログに残せるか

### 編集失敗とみなす例

- どのLPにも使える一般論のブロックを末尾に足しただけ
- heroの雰囲気と違う強い煽りコピーを入れる
- CTAボタンを増やしすぎて導線が散る
- 既存画像やレイアウトを覆って読みづらくする
- GA4ではCTA課題なのに、関係の薄い装飾だけ変更する
- draft用の説明文が本番コンテンツに残る
  ...
  "changes": [
    {
      ...
      "before_text": "置換前の文言または要約",
      "after_text": "置換後に入れる自然な文言",
      "ga4_reason": "この変更がどのGA4課題に対応するか",
      "naturalness_check": "既存トーン・文量・スマホ表示に合う理由",
...
- 本番フォルダを触らない
- 変更前後を必ずログに残す
- `before_text` / `after_text` / `ga4_reason` / `naturalness_check` を出させる
- 置換できない場合は安全に差し込みへ戻す
- 既存LPの部品に自然に溶け込む置換を優先し、汎用的なAI説明ブロックを避ける
