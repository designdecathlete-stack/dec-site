-- ailp-manager: LP-scoped mapping for client-provided GA4/GTM event names, including scroll depth.

alter table public.lp_analytics_settings
  add column if not exists event_mappings jsonb not null default '{}'::jsonb;

comment on column public.lp_analytics_settings.event_mappings is 'LP-scoped GA4 event name mapping. Keys: cta, line, reservation, scroll_25, scroll_50, scroll_75, scroll_90. Values are arrays of client-provided GA4 event names.';
...
with settings_map as (
  select
    ...
    coalesce(settings.event_mappings, '{}'::jsonb) as event_mappings,
    coalesce(settings.event_mappings -> 'cta', '["cta_click", "lp_cta_click"]'::jsonb) as cta_events,
    coalesce(settings.event_mappings -> 'line', '["line_click", "line_tap", "click_line"]'::jsonb) as line_events,
    coalesce(settings.event_mappings -> 'reservation', '["reservation_click", "booking_click", "reserve_click", "hotpepper_click"]'::jsonb) as reservation_events,
    coalesce(settings.event_mappings -> 'scroll_25', '["scroll_25"]'::jsonb) as scroll_25_events,
    coalesce(settings.event_mappings -> 'scroll_50', '["scroll_50"]'::jsonb) as scroll_50_events,
    coalesce(settings.event_mappings -> 'scroll_75', '["scroll_75"]'::jsonb) as scroll_75_events,
    coalesce(settings.event_mappings -> 'scroll_90', '["scroll", "scroll_90"]'::jsonb) as scroll_90_events
...
event_rollups as (
  select
    ...
    sum(e.event_count) filter (where e.metric_date >= current_date - interval '29 days' and exists (select 1 from jsonb_array_elements_text(sm.cta_events) names(name) where names.name = e.event_name))::bigint as cta_clicks_30d,
    sum(e.event_count) filter (where e.metric_date >= current_date - interval '29 days' and exists (select 1 from jsonb_array_elements_text(sm.line_events) names(name) where names.name = e.event_name))::bigint as line_clicks_30d,
    sum(e.event_count) filter (where e.metric_date >= current_date - interval '29 days' and exists (select 1 from jsonb_array_elements_text(sm.reservation_events) names(name) where names.name = e.event_name))::bigint as reservation_clicks_30d,
    sum(e.event_count) filter (where e.metric_date >= current_date - interval '29 days' and exists (select 1 from jsonb_array_elements_text(sm.scroll_25_events) names(name) where names.name = e.event_name))::bigint as scroll_25_30d,
    sum(e.event_count) filter (where e.metric_date >= current_date - interval '29 days' and exists (select 1 from jsonb_array_elements_text(sm.scroll_50_events) names(name) where names.name = e.event_name))::bigint as scroll_50_30d,
    sum(e.event_count) filter (where e.metric_date >= current_date - interval '29 days' and exists (select 1 from jsonb_array_elements_text(sm.scroll_75_events) names(name) where names.name = e.event_name))::bigint as scroll_75_30d,
    sum(e.event_count) filter (where e.metric_date >= current_date - interval '29 days' and exists (select 1 from jsonb_array_elements_text(sm.scroll_90_events) names(name) where names.name = e.event_name))::bigint as scroll_90_30d,
    max(e.synced_at) as last_event_synced_at
  from public.ga4_daily_events e
  join settings_map sm on sm.lp_project_id = e.lp_project_id
...
select
  ...
  case when not analytics.is_active then 'disabled' when analytics.ga4_property_id is not null and analytics.ga4_page_path is not null and analytics.ga4_page_path <> '' then 'configured' when analytics.ga4_property_id is not null or analytics.ga4_page_path is not null then 'partial' else 'missing' end as ga4_connection_status,
  ...
  coalesce(event_rollups.scroll_25_30d, 0) as scroll_25_30d,
  coalesce(event_rollups.scroll_50_30d, 0) as scroll_50_30d,
  coalesce(event_rollups.scroll_75_30d, 0) as scroll_75_30d,
  coalesce(event_rollups.scroll_90_30d, 0) as scroll_90_30d,
  case when coalesce(metric_rollups.sessions_30d, 0) > 0 then round((coalesce(event_rollups.scroll_25_30d, 0)::numeric / metric_rollups.sessions_30d::numeric) * 100, 2) else null end as scroll_25_rate_30d,
  case when coalesce(metric_rollups.sessions_30d, 0) > 0 then round((coalesce(event_rollups.scroll_50_30d, 0)::numeric / metric_rollups.sessions_30d::numeric) * 100, 2) else null end as scroll_50_rate_30d,
  case when coalesce(metric_rollups.sessions_30d, 0) > 0 then round((coalesce(event_rollups.scroll_75_30d, 0)::numeric / metric_rollups.sessions_30d::numeric) * 100, 2) else null end as scroll_75_rate_30d,
  case when coalesce(metric_rollups.sessions_30d, 0) > 0 then round((coalesce(event_rollups.scroll_90_30d, 0)::numeric / metric_rollups.sessions_30d::numeric) * 100, 2) else null end as scroll_90_rate_30d
from public.lp_projects lp
