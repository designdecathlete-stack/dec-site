...
function enhancedExecution(){
  ...
  const successfulDraft=latestApply?.status==='succeeded'
  const draftArtifact=successfulDraft?(artifactForJob(latestApply)||latestDraftArtifact()):null
  ...
  const draftUrl=successfulDraft?(draftArtifact?.preview_url||latestApply?.preview_url||''):''
  const githubUrl=successfulDraft?(draftArtifact?.metadata?.github_branch_url||latestApply?.payload?.github_branch_url||''):''
