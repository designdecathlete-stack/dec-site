...
async function requestCodexProposal(force=false){
  ...
  if(running){
    notice('Codex提案作成は実行中です。完了まで再作成せず、ステータスを更新します。')
    await loadDetailData(true)
    return
  }
  if(analysis&&!force){
    notice('保存済みの最新Codex提案を表示しました。再作成する場合だけ「強制再作成」を押してください。')
    await loadDetailData(true)
    return
  }
  return enqueueLpJob('propose_improvements',{executor:'codex',force:Boolean(force),knowledge_files:['docs/ai-proposal-prompt.md','docs/ai-html-edit-prompt.md','docs/ai-improvement-logic.md','docs/ga4-scoring-logic.md']})
}
...
function workflowStatusPanel(activeStage=''){
  ...
  const draftUrl=draftArtifact?.preview_url||displayDraftJob?.preview_url||''
...
function enhancedExecution(){
  ...
  const draftUrl=successfulDraft?(draftArtifact?.preview_url||displayDraftJob?.preview_url||''):''
...
function enhancedButtons(){
  document.querySelectorAll('[data-action]').forEach(button=>{
    button.onclick=event=>{
      ...
      else if(action==='vps-propose'){ requestCodexProposal(false) }
      else if(action==='vps-propose-force'){ requestCodexProposal(true) }
