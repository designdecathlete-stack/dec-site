...
async function publishApprovedDraft(){
  ...
  if(!approved){ notice('先に「このdraftでOK」で承認してください。'); return }
...
function enhancedExecution(){
  ...
  const draftArtifact=artifactForJob(latestApply)||latestDraftArtifact()
  const usage=usageForJob(latestApply)
  const draftVersion=versionForJob(latestApply,draftArtifact)
  ...
  const executionRecommendations=Array.isArray(latestApply?.payload?.override_recommendations)&&latestApply.payload.override_recommendations.length?analysisListItems(latestApply.payload.override_recommendations):recommendations
  const sourceAnalysisId=latestApply?.payload?.ai_analysis_result_id||analysis?.id||''
  const approved=Boolean(draftVersion?.change_summary&&String(draftVersion.change_summary).startsWith('[承認済みdraft]'))
  return enhancedDetailContext('execution')+`${detailState.error?`<div class="ga4-empty">${escapeHtml(detailState.error)}</div>`:''}<div class="heading-row"><div><div class="eyebrow">IMPLEMENTATION</div><h1>修正実行</h1><p class="page-sub">AI提案または編集済み改善案をdraftへ反映し、preview確認後に承認済みdraftだけ本番へ反映できます。</p></div><div class="ga4-actions"><button class="secondary" onclick="go('proposals')">改善点を確認する</button><button class="secondary" data-action="detail-refresh">状態を更新</button></div></div><div class="dashboard-grid"><section class="panel section-card"><h2>draft生成ステータス</h2><div class="ga4-field-list"><span>状態 <b>${statusBadge(status.label,status.tone)}</b></span><span>詳細 <b>${escapeHtml(status.detail)}</b></span><span>最新job <b>${latestApply?escapeHtml(latestApply.id.slice(0,8)):'--'}</b></span><span>作成 <b>${escapeHtml(formatDisplayDate(latestApply?.created_at))}</b></span><span>AIコスト <b>${escapeHtml(costLabel(usage))}</b></span></div>${latestApply?.status==='failed'?`<div class="ga4-empty">失敗理由：${escapeHtml(latestApply.error_message||'不明')}</div>`:''}</section><section class="panel section-card"><h2>preview確認</h2><div class="ga4-field-list compact"><span>preview URL <b>${draftUrl?`<a href="${escapeHtml(draftUrl)}" target="_blank" rel="noopener">開く</a>`:'未発行'}</b></span><span>GitHub branch <b>${githubUrl?`<a href="${escapeHtml(githubUrl)}" target="_blank" rel="noopener">開く</a>`:'未作成'}</b></span><span>commit <b>${escapeHtml(draftArtifact?.commit_sha||latestApply?.commit_sha||'--')}</b></span><span>本番反映 <b>${approved?'承認済み / 反映可能':'承認前 / 未反映'}</b></span></div><div class="ga4-actions stack"><button class="secondary" data-action="vps-propose">Codex提案を作成</button><button class="primary" data-action="vps-apply-draft" ${analysis?'':'disabled'}>AI提案をdraftへ反映</button><button class="primary" data-action="approve-draft" ${latestApply?.status==='succeeded'&&!approved?'':'disabled'}>${approved?'承認済み':'このdraftでOK'}</button><button class="primary" data-action="publish-approved-draft" ${approved?'':'disabled'}>本番へ反映</button></div></section><section class="panel section-card"><h2>反映された改善内容</h2>${edits.length?`<ul class="mini-list">${edits.map(item=>`<li><span>${escapeHtml(item.target_area||item.type||'edit')}<br><small>${escapeHtml(item.type||'')}</small></span><b>${escapeHtml(item.title||item.label||item.after||'変更内容未記録')}</b></li>`).join('')}</ul>`:`<div class="ga4-empty">まだ反映ログがありません。</div>`}</section><section class="panel section-card"><h2>draftへ渡した修正対象</h2>${sourceAnalysisId?`<div class="version-commit"><small>AI分析: ${escapeHtml(String(sourceAnalysisId).slice(0,8))}</small></div>`:''}${executionRecommendations.length?`<ul class="mini-list">${executionRecommendations.map(item=>`<li><span>${escapeHtml(item.title)}</span><b>${escapeHtml(item.body||item.detail||item.expected_effect||'')}</b></li>`).join('')}</ul>`:`<div class="ga4-empty">先に「AI提案を作成」を実行してください。</div>`}</section></div><section class="panel table-panel"><div class="heading-row"><div><h2>VPSジョブ</h2><p class="page-sub">処理中、成功、失敗理由、commitを確認できます。</p></div></div>${jobs.length?`<table><thead><tr><th>種別</th><th>状態</th><th>結果 / 失敗理由</th><th>Preview</th><th>作成</th></tr></thead><tbody>${jobs.slice(0,10).map(job=>`<tr><td><div class="lp-name">${escapeHtml(job.job_type)}</div></td><td>${statusBadge(jobStatusLabel(job.status),jobStatusTone(job.status))}</td><td>${escapeHtml(job.result_summary||job.error_message||'処理待ち')}</td><td>${job.preview_url?`<a href="${escapeHtml(job.preview_url)}" target="_blank" rel="noopener">開く</a>`:'--'}<div class="ga4-field-list compact"><span>${escapeHtml(job.git_branch||'--')}</span><span>${escapeHtml(job.commit_sha||'--')}</span></div></td><td>${escapeHtml(formatDisplayDate(job.created_at))}</td></tr>`).join('')}</tbody></table>`:`<div class="ga4-empty">VPSジョブはまだありません。</div>`}</section>`
}
